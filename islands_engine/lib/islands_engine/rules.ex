defmodule IslandsEngine.Rules do
  ...
  def check(%Rules{state: :player1_turn} = rules, {:win_check, win_or_nowin}) do
    case win_or_nowin do
      :no_win -> {:ok, rules}
  ...
  def check(%Rules{state: :player2_turn} = rules, {:win_check, win_or_nowin}) do
    case win_or_nowin do
      :no_win -> {:ok, rules}
      :win -> {:ok, %Rules{rules | state: :game_over}}
    end
